...
def _provider(source: SourceDefinition) -> ConnectionProvider:
    if source.source_type == SourceType.MICROSOFT_EXCEL:
        return ConnectionProvider.MICROSOFT_365
...
def connection_readiness(
    ...
    *,
    ...
    """Return credential/location readiness without reading or returning a secret value.

    This is a local configuration/readiness check only. `READY_TO_TEST` means AnalystWatch has
    enough local configuration to attempt a provider request; it does not claim that the token is
    valid, that provider access succeeds, or that any tenant resource has been verified.
    """
