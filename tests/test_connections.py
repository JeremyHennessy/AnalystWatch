...
from analystwatch.connections import (
    ConnectionProvider,
...
def test_microsoft_connection_readiness_requires_credential_reference() -> None:
    ...
    assert readiness.provider == ConnectionProvider.MICROSOFT_365
...
def test_ready_to_test_means_local_configuration_only_and_never_serializes_secret() -> None:
    ...
    assert token not in readiness.model_dump_json()
    assert "ANALYSTWATCH_MICROSOFT_AUTHORIZATION" not in readiness.model_dump_json()
...
@pytest.mark.parametrize(
    ...
    [
        (
            ...
            ConnectionProvider.MICROSOFT_365,
        ),
        (
            _google_source(
